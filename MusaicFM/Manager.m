...
- (void)performSpotifyUserAlbums:(void (^)(NSArray *items))completion
  ...
  NSURLRequest * (^create)(NSInteger offset) =
      ^NSURLRequest *(NSInteger offset) {
        NSURLComponents *offsetComponents = [Factory spotifyAlbums];
        offsetComponents.queryItems = @[
  ...
  void (^initialCompletion)(NSArray *responses) = ^void(NSArray *responses) {
    ...
    next.count ? [self performSpotifyTokenRequest:next.copy
                            withCompletionHandler:finalCompletion
                                       andFailure:failure]
               : finalCompletion(total.copy);
  };

  [self performSpotifyTokenRequest:@[ create(0) ]
             withCompletionHandler:initialCompletion
                        andFailure:failure];
}
...
- (void)performSpotifyReleases:(void (^)(NSArray *items))completion
  ...
  NSURLRequest * (^create)(NSInteger offset) =
      ^NSURLRequest *(NSInteger offset) {
        ...
        NSMutableURLRequest *request =
            [NSMutableURLRequest requestWithURL:offsetComponents.URL];
        return request.copy;
      };
  ...
  void (^initialCompletion)(NSArray *responses) = ^void(NSArray *responses) {
    ...
    [self performSpotifyTokenRequest:nextRequests.copy
               withCompletionHandler:finalCompletion
                          andFailure:failure];
  };
...
- (void)performSpotifyLikedSongs:(void (^)(NSArray *items))completion
  ...
  void (^finalCompletion)(NSArray *responses) = ^void(NSArray *responses) {
    ...
    for (NSDictionary *items in total) {
      for (NSDictionary *itemDict in items[@"items"]) {
        [uniqueAlbums setObject:itemDict[@"track"][@"album"]
                         forKey:itemDict[@"track"][@"album"][@"id"]];
      }
    }
...
- (void)performDeezerPlaylists:(void (^)(NSArray *items))completion
  ...
  NSURLComponents *components =
      [Factory deezerUserPlaylists:self.preferences.deezerUserId];
  NSURLRequest *request = [NSURLRequest requestWithURL:components.URL];
  __weak typeof(self) weakSelf = self;

  [self performRequest:request
      withCompletionHandler:^(NSDictionary *response) {
        NSArray *playlists = response[@"data"];
        NSMutableArray *requests = [NSMutableArray array];
        for (NSDictionary *playlist in playlists) {
          NSURLComponents *components =
              [Factory deezerPlaylist:playlist[@"id"]];
          [requests addObject:[NSURLRequest requestWithURL:components.URL]];
        }
        ...
        for (NSURLRequest *request in requests) {
          [weakSelf performRequest:request
              withCompletionHandler:^(NSDictionary *response) {
                [totalResponses
                    addObjectsFromArray:response[@"tracks"][@"data"]];
                taskCount -= 1;
                if (taskCount == 0) {
                  NSArray *artworks = [Parser parseDeezerItems:totalResponses];
                  Preferences *preferences = [Preferences preferences];
                  preferences.artworks = artworks;
                  [preferences synchronize];
                  if (completion)
...
- (void)performSpotifyToken:(NSString *)code
  ...
  request.HTTPBody = [[[self class] NSStringFromQueryParameters:body]
...
- (void)performLastfmTag:(void (^)(NSArray *items))completion
              andFailure:(void (^)(NSError *error))failure {
  NSMutableArray *paramter =
      @[
        [NSURLQueryItem queryItemWithName:@"tag"
                                    value:self.preferences.lastfmTag],
...
- (void)performLastfmWeekly:(void (^)(NSArray *items))completion
  ...
  NSMutableArray *paramter =
      @[
        [NSURLQueryItem queryItemWithName:@"user"
                                    value:self.preferences.lastfmUser],
  ...
  case Weekly7Days:
    [paramter addObject:[NSURLQueryItem queryItemWithName:@"period"
                                                    value:@"7day"]];
    break;
  ...
  [self performRequest:request
      withCompletionHandler:^(NSDictionary *response) {
        NSArray *artworks =
            [Parser parseItems:response[@"topalbums"][@"album"]];
        Preferences *preferences = [Preferences preferences];
        preferences.artworks = artworks;
        [preferences synchronize];
        if (completion)
...
- (void)performSpotifyTokenRequest:(NSArray *)requests
  ...
  void (^recovery)(void) = ^{
    NSData *authData =
        [[NSString stringWithFormat:@"%@:%@", spotifyClientId, spotifySecretId]
            dataUsingEncoding:NSUTF8StringEncoding];
    NSString *auth =
    ...
    NSMutableURLRequest *recovery =
        [NSMutableURLRequest requestWithURL:components.URL];
    recovery.HTTPMethod = @"POST";
    recovery.HTTPBody = [[[self class] NSStringFromQueryParameters:body]
        dataUsingEncoding:NSUTF8StringEncoding];
    [recovery addValue:@"application/x-www-form-urlencoded; charset=utf-8"
        forHTTPHeaderField:@"Content-Type"];
    ...
    [weakSelf performRequest:recovery.copy
        withCompletionHandler:^(NSDictionary *response) {
          ...
          if (error) {
            ...
            [weakSelf.preferences synchronize];
            [weakSelf performSpotifyTokenRequest:requests
  ...
  void (^internalCompletion)(NSDictionary *response) =
      ^void(NSDictionary *response) {
  ...
  for (NSURLRequest *request in requests) {
    ...
    [authRequest setValue:[NSString
                              stringWithFormat:@"Bearer %@",
                                               self.preferences.spotifyToken]
        forHTTPHeaderField:@"Authorization"];
    [authRequest setValue:@"application/x-www-form-urlencoded; charset=utf-8"
        forHTTPHeaderField:@"Content-Type"];
    [self performRequest:authRequest
        withCompletionHandler:internalCompletion
                   andFailure:failure];
  }
}
...
- (NSURLSessionDataTask *)performRequest:(NSURLRequest *)request
                   withCompletionHandler:
                       (void (^)(NSDictionary *response))completion
                              andFailure:(void (^)(NSError *error))failure {
      ...
      [self.session dataTaskWithRequest:request
                      completionHandler:^(NSData *data, NSURLResponse *response,
                        ...
                        if (error && failure) {
                          ...
                          completion([NSJSONSerialization
