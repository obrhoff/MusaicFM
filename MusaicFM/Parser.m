...
+ (NSArray *)parseSpotifyItems:(NSArray *)responseDicts {
  ...
  for (NSDictionary *albumDict in responseDicts) {
    ...
    NSArray *images = [albumDict[@"images"]
        sortedArrayUsingComparator:^NSComparisonResult(NSDictionary *first,
                                                       NSDictionary *second) {
          CGFloat firstCompare =
              fabs([first[@"width"] floatValue] - targetSize);
          CGFloat secondCompare =
              fabs([second[@"width"] floatValue] - targetSize);
...
+ (NSArray *)parseDeezerItems:(NSArray *)itemDicts {
  NSMutableArray *items = [NSMutableArray array];
  for (NSDictionary *itemDict in itemDicts) {
    NSURL *artworkUrl = [NSURL URLWithString:itemDict[@"picture_xl"]];
    if (!artworkUrl) {
      continue;
    }
    Artwork *artwork = [Artwork new];
    artwork.artworkUrl = artworkUrl;
    artwork.album = itemDict[@"title"];
    artwork.artist = itemDict[@"creator"][@"name"];
    artwork.url = [NSURL URLWithString:itemDict[@"link"]];
    [items addObject:artwork];
  }
  return [NSSet setWithArray:items.copy].allObjects;
}
