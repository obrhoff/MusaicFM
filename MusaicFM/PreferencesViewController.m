...
@property(nonatomic, readwrite, weak)
    IBOutlet NSTextField *delayDescriptionLabel;
...
@property(nonatomic, readwrite, weak)
    IBOutlet NSTextField *deezerUserIdTextField;

@property(nonatomic, readwrite, weak) IBOutlet NSTextField *lastFMPeriodLabel;
@property(nonatomic, readwrite, weak)
    IBOutlet NSButton *lastfmPlayedAlbumsRadio;
@property(nonatomic, readwrite, weak) IBOutlet NSButton *lastFmTagRadio;
@property(nonatomic, readwrite, weak)
    IBOutlet NSButton *spotifyPlayedAlbumsRadio;
@property(nonatomic, readwrite, weak) IBOutlet NSButton *spotifyNewRadio;
@property(nonatomic, readwrite, weak)
    IBOutlet NSButton *spotifyFavoriteSongsRadio;
@property(nonatomic, readwrite, weak) IBOutlet NSButton *spotifySignIn;
@property(nonatomic, readwrite, weak) IBOutlet NSButton *deezerRadio;

@property(nonatomic, readwrite, weak)
    IBOutlet NSPopUpButton *lastFmWeeklyButton;
...
- (void)configure {
  ...
  self.deezerUserIdTextField.delegate = self;

  self.previewView.layer.cornerRadius = 8;

  [self.lastFmWeeklyButton addItemsWithTitles:@[
    [NSBundle localizedString:@"lastfm_fetch_period_overall"],
...
- (void)updateView {
  ...
  for (NSButton *button in self.radioButtons) {
    button.state =
        button == radioButton ? NSControlStateValueOn : NSControlStateValueOff;
  }

  for (NSButton *button in @[
         self.spotifyPlayedAlbumsRadio, self.spotifyNewRadio,
  ...
  self.deezerUserIdTextField.stringValue = self.preferences.deezerUserId;

  self.titleLabel.stringValue = [NSBundle localizedString:@"app_title"];
  self.descriptionLabel.stringValue =
      [NSBundle localizedString:@"app_title_description"];
  self.rowLabel.stringValue = [NSBundle localizedString:@"rows"];

  self.delayLabel.stringValue = [NSBundle localizedString:@"delay"];

  self.spotifySignIn.title =
      self.preferences.spotifyToken.length
          ? [NSBundle localizedString:@"spotify_sign_out"]
          : [NSBundle localizedString:@"spotify_sign_in"];

  self.spotifyNewRadio.title =
      [NSBundle localizedString:@"spotify_new_releases"];
  self.spotifyFavoriteSongsRadio.title =
      [NSBundle localizedString:@"spotify_liked_songs"];
  self.spotifyPlayedAlbumsRadio.title =
      [NSBundle localizedString:@"spotify_albums"];
  self.lastFMPeriodLabel.stringValue =
      [NSBundle localizedString:@"lastfm_fetch_period_overall_title"];
  self.lastFmTagRadio.title = [NSBundle localizedString:@"lastfm_tags"];
  self.lastfmPlayedAlbumsRadio.title =
      [NSBundle localizedString:@"lastfm_user_albums"];
  self.deezerRadio.title = @"Deezer Playlists";
  self.delayDescriptionLabel.stringValue =
      [NSString stringWithFormat:[NSBundle localizedString:@"delay_in_seconds"],
                                 self.delaySlider.integerValue];
...
- (IBAction)loginSpotify:(NSButton *)sender {
  if (self.preferences.spotifyToken.length) {
    ...
    NSURLRequest *request =
        [NSURLRequest requestWithURL:[Factory spotifyAuthentification].URL];
...
- (void)didChangeText {
  ...
  self.preferences.deezerUserId = self.deezerUserIdTextField.stringValue;
  [self updateView];
}
...
- (void)webView:(WKWebView *)webView
  ...
  NSURLComponents *components =
      [NSURLComponents componentsWithURL:[navigationAction request].URL
                 resolvingAgainstBaseURL:NO];
  NSURLComponents *callbackComponents =
      [NSURLComponents componentsWithString:spotifyRedirectUrl];
  ...
  NSPredicate *filter =
      [NSPredicate predicateWithFormat:@"name == %@", @"code"];
  NSString *code = [[components.queryItems
                        filteredArrayUsingPredicate:filter].firstObject value];
...
- (NSArray *)radioButtons {
  return @[
    self.lastfmPlayedAlbumsRadio, self.lastFmTagRadio,
    self.spotifyPlayedAlbumsRadio, self.spotifyNewRadio,
    self.spotifyFavoriteSongsRadio, self.deezerRadio
  ];
}
